...
func setup(args []string) error {
	...
	log := logger.CreateQuietLogger()

	configuration, err = GetConfiguration(cliArgs, &log)
...
func TestSpecificationFileURL(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml"})
...
func TestOverrideForMFADecision(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml", "--mfa"})
	assert.Equal(t, true, configuration.DefaultDecisionForMFA)
}

func TestNoMFADecision(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--sandbox"})
	assert.Equal(t, false, configuration.DefaultDecisionForMFA)
}
...
func TestCLIOverrideForRegion(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml", "--region=ap-southeast-1"})
...
func TestCLIOverrideForDurationForMFA(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml", "--duration=1600"})
...
func TestTooBigDurationForMFA(t *testing.T) {
	err := setup([]string{"cmd", "validate", "some_path", "--duration=600000000"})
	assert.NotNil(t, err)
}

func TestTooSmallDurationForMFA(t *testing.T) {
	err := setup([]string{"cmd", "validate", "some_path", "--duration=-1"})
	assert.NotNil(t, err)
}

func TestZeroDurationForMFA(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--duration=0", "--sandbox"})
	assert.Equal(t, int64(0), configuration.DefaultDurationForMFA)
}
...
func TestCLIOverrideForProfile(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml", "--profile=cliProfile"})
...
func TestCLIOverrideForVerbosity(t *testing.T) {
	setup([]string{"cmd", "validate", "some_path", "--config=test_resources/test_config.yaml", "--verbosity=INFO"})
